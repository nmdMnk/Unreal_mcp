...
{ scenario: 'Setup: create audio component on actor', toolName: 'manage_audio', arguments: { action: 'create_audio_component', soundPath: SOUND_CUE, actorName: FADE_ACTOR_NAME }, expected: 'success' },

// Add nodes to SoundCue for connect test
// C++ handler reads nodeType.ToLower() and matches 'random', 'waveplayer', 'modulator', etc.
{ scenario: 'Setup: add random node to sound cue', toolName: 'manage_audio', arguments: { action: 'add_cue_node', assetPath: SOUND_CUE, nodeType: 'random', nodeName: 'TestRandomNode' }, expected: 'success|already exists' },
{ scenario: 'Setup: add wave player node to sound cue', toolName: 'manage_audio', arguments: { action: 'add_cue_node', assetPath: SOUND_CUE, nodeType: 'wave_player', nodeName: 'TestWavePlayer' }, expected: 'success|already exists' },

// MetaSound nodes — C++ maps nodeType to 3-part class names:
// 'sine' → {UE, Sine, Audio}, 'multiply'/'gain' → {UE, Multiply, Float}
{ scenario: 'Setup: add sine node to metasound', toolName: 'manage_audio', arguments: { action: 'add_metasound_node', assetPath: METASOUND, nodeType: 'sine', nodeName: 'TestSineNode' }, expected: 'success', captureResult: { key: 'sineNodeId', fromField: 'result.nodeId' } },
{ scenario: 'Setup: add gain node to metasound', toolName: 'manage_audio', arguments: { action: 'add_metasound_node', assetPath: METASOUND, nodeType: 'gain', nodeName: 'TestGainNode' }, expected: 'success', captureResult: { key: 'gainNodeId', fromField: 'result.nodeId' } },
...
// === CREATE ===
{ scenario: 'CREATE: create_sound_cue', toolName: 'manage_audio', arguments: { action: 'create_sound_cue', name: `Testsound_cue_${ts}`, path: '/Game/MCPTest' }, expected: 'success|already exists' },
...
{ scenario: 'CREATE: create_sound_class', toolName: 'manage_audio', arguments: { action: 'create_sound_class', name: `Testsound_class_${ts}`, path: '/Game/MCPTest' }, expected: 'success|already exists' },

// === PLAYBACK === (uses ResolveSoundAsset - accepts package path)
{ scenario: 'PLAYBACK: play_sound_at_location', toolName: 'manage_audio', arguments: { action: 'play_sound_at_location', soundPath: SOUND_CUE, location: [0, 0, 0] }, expected: 'success' },
...
{ scenario: 'PLAYBACK: play_sound_attached', toolName: 'manage_audio', arguments: { action: 'play_sound_attached', soundPath: SOUND_CUE, actorName: FADE_ACTOR_NAME }, expected: 'success' },
...
{ scenario: 'CONFIG: set_sound_mix_class_override', toolName: 'manage_audio', arguments: { action: 'set_sound_mix_class_override', mixName: 'TestSoundMix', soundClassName: 'TestSoundClass', volume: 0.8, pitch: 1.0 }, expected: 'success' },
{ scenario: 'DELETE: clear_sound_mix_class_override', toolName: 'manage_audio', arguments: { action: 'clear_sound_mix_class_override', mixName: 'TestSoundMix', soundClassName: 'TestSoundClass' }, expected: 'success' },
...
{ scenario: 'ACTION: fade_sound_in', toolName: 'manage_audio', arguments: { action: 'fade_sound_in', actorName: FADE_ACTOR_NAME, fadeTime: 1.0 }, expected: 'success' },
{ scenario: 'ACTION: fade_sound_out', toolName: 'manage_audio', arguments: { action: 'fade_sound_out', actorName: FADE_ACTOR_NAME, fadeTime: 1.0 }, expected: 'success' },
// fade_sound: C++ reads soundName (preferred) or actorName
{ scenario: 'ACTION: fade_sound', toolName: 'manage_audio', arguments: { action: 'fade_sound', soundName: FADE_ACTOR_NAME, fadeTime: 1.0, targetVolume: 0.5 }, expected: 'success' },
...
// === ATTENUATION ===
{ scenario: 'CONFIG: set_sound_attenuation', toolName: 'manage_audio', arguments: { action: 'set_sound_attenuation', name: `TestSetAttenuation_${ts}`, innerRadius: 400, falloffDistance: 3600, path: TEST_FOLDER, save: false }, expected: 'success', assertions: [{ path: 'structuredContent.result.existsAfter', equals: true, label: 'attenuation asset configured in editor asset registry' }] },
...
{ scenario: 'ADD: add_cue_node', toolName: 'manage_audio', arguments: { action: 'add_cue_node', assetPath: SOUND_CUE, nodeType: 'modulator', nodeName: `Testmod_${ts}` }, expected: 'success|already exists' },
...
{ scenario: 'CONFIG: set_cue_concurrency', toolName: 'manage_audio', arguments: { action: 'set_cue_concurrency', assetPath: SOUND_CUE }, expected: 'success' },
...
{ scenario: 'ADD: add_metasound_node', toolName: 'manage_audio', arguments: { action: 'add_metasound_node', assetPath: METASOUND, nodeType: 'add', nodeName: `Testadd_${ts}` }, expected: 'success', captureResult: { key: 'addNodeId', fromField: 'result.nodeId' } },
...
{ scenario: 'CONFIG: set_class_properties', toolName: 'manage_audio', arguments: { action: 'set_class_properties', assetPath: SOUND_CLASS, volume: 0.8, pitch: 1.0 }, expected: 'success' },
