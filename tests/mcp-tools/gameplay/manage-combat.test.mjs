...
const testCases = [
  ...
  // === WEAPON BASE ===
  {
    ...
    arguments: { action: 'create_weapon_blueprint', name: WEAPON_NAME, path: TEST_FOLDER, parentClass: 'Actor', weaponType: 'Rifle', baseDamage: 37, fireRate: 480, range: 9000, spread: 1.5 },
    expected: 'success',
  ...
  // === PROJECTILES ===
  {
    ...
    toolName: 'manage_combat',
    arguments: { action: 'create_projectile_blueprint', name: PROJECTILE_NAME, path: TEST_FOLDER, parentClass: 'Actor', projectileSpeed: 6000, projectileGravityScale: 0, collisionRadius: 8 },
    expected: 'success',
    captureResult: { key: 'projectilePath', fromField: 'result.blueprintPath' },
    assertions: [{ path: 'structuredContent.result.existsAfter', equals: true, label: 'projectile blueprint exists after creation' }]
  },
  ...
  { scenario: 'ACTION: setup_hitbox_component', toolName: 'manage_combat', arguments: { action: 'setup_hitbox_component', blueprintPath: weaponPath, hitboxType: 'Box', hitboxBoneName: 'spine_03', isDamageZoneHead: true, damageMultiplier: 2 }, expected: 'success', assertions: [{ path: 'structuredContent.result.hitboxType', equals: 'Box', label: 'hitbox type applied' }] },

  // === WEAPON FEATURES ===
  { scenario: 'ACTION: setup_reload_system', toolName: 'manage_combat', arguments: { action: 'setup_reload_system', blueprintPath: weaponPath, magazineSize: 24, reloadTime: 2 }, expected: 'success', assertions: [{ path: 'structuredContent.result.magazineSize', equals: 24, label: 'magazine size applied' }] },
  { scenario: 'ACTION: setup_ammo_system', toolName: 'manage_combat', arguments: { action: 'setup_ammo_system', blueprintPath: weaponPath, ammoType: 'Rifle', maxAmmo: 120, startingAmmo: 48, ammoPerShot: 1, infiniteAmmo: false }, expected: 'success', assertions: [{ path: 'structuredContent.result.maxAmmo', equals: 120, label: 'max ammo applied' }] },
  ...
  { scenario: 'ACTION: setup_weapon_switching', toolName: 'manage_combat', arguments: { action: 'setup_weapon_switching', blueprintPath: weaponPath, switchInTime: 0.5, switchOutTime: 0.25 }, expected: 'success', assertions: [{ path: 'structuredContent.result.switchInTime', equals: 0.5, label: 'weapon switch-in time applied' }] },

  // === EFFECTS ===
  { scenario: 'CONFIG: configure_muzzle_flash', toolName: 'manage_combat', arguments: { action: 'configure_muzzle_flash', blueprintPath: weaponPath, muzzleFlashScale: 1.5 }, expected: 'success', assertions: [{ path: 'structuredContent.result.scale', equals: 1.5, label: 'muzzle flash scale applied' }] },
  { scenario: 'CONFIG: configure_tracer', toolName: 'manage_combat', arguments: { action: 'configure_tracer', blueprintPath: weaponPath, tracerSpeed: 15000 }, expected: 'success', assertions: [{ path: 'structuredContent.result.tracerSpeed', equals: 15000, label: 'tracer speed applied' }] },
  ...
  { scenario: 'CONFIG: configure_shell_ejection', toolName: 'manage_combat', arguments: { action: 'configure_shell_ejection', blueprintPath: weaponPath, shellEjectionForce: 400, shellLifespan: 3 }, expected: 'success', assertions: [{ path: 'structuredContent.result.ejectionForce', equals: 400, label: 'shell ejection force applied' }] },
  ...
  { scenario: 'CONFIG: configure_hit_reaction', toolName: 'manage_combat', arguments: { action: 'configure_hit_reaction', blueprintPath: weaponPath, hitReactionStunTime: 0.5 }, expected: 'success', assertions: [{ path: 'structuredContent.result.stunTime', equals: 0.5, label: 'hit reaction stun time applied' }] },
  { scenario: 'ACTION: setup_parry_block_system', toolName: 'manage_combat', arguments: { action: 'setup_parry_block_system', blueprintPath: weaponPath, parryWindowStart: 0, parryWindowEnd: 0.25, blockDamageReduction: 0.5, blockStaminaCost: 12 }, expected: 'success', assertions: [{ path: 'structuredContent.result.blockDamageReduction', equals: 0.5, label: 'block reduction applied' }] },
  { scenario: 'CONFIG: configure_weapon_trails', toolName: 'manage_combat', arguments: { action: 'configure_weapon_trails', blueprintPath: weaponPath, weaponTrailStartSocket: 'TrailStart', weaponTrailEndSocket: 'TrailEnd' }, expected: 'success', assertions: [{ path: 'structuredContent.result.trailStartSocket', equals: 'TrailStart', label: 'weapon trail start socket applied' }] },
  ...
  { scenario: 'CREATE: create_shield', toolName: 'manage_combat', arguments: { action: 'create_shield', blueprintPath: weaponPath, shieldAmount: 60, maxShield: 120, shieldRegenRate: 6, shieldRegenDelay: 3 }, expected: 'success', assertions: [{ path: 'structuredContent.result.maxShield', equals: 120, label: 'max shield applied' }] },
